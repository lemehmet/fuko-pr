...
def _items(*texts, source="resolved_thread"):
...
def test_same_text_different_source_is_not_a_duplicate():
    ...
    to_embed, skipped, deferred = partition(items, {("a", "resolved_thread")})
...
def test_cap_counts_only_new_items():
    existing = {("a", "resolved_thread"), ("b", "resolved_thread")}
...
def test_totals_always_account_for_every_item():
    ...
    to_embed, skipped, deferred = partition(items, {("d", "resolved_thread")}, max_new=1)
