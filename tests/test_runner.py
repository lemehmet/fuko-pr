...
def test_resolve_actor_app_token_403_returns_token_surrogate(monkeypatch):
    # GitHub App installation tokens 403 on GET /user; identity falls back to a
    # per-token surrogate so two apps still resolve to distinct identities.
    monkeypatch.setattr(
        runner.httpx, "get", lambda url, headers=None, timeout=None: _Resp({}, status=403)
    )
...
def test_resolve_branch_identities_activates_for_two_app_tokens(monkeypatch):
    # End-to-end: two app installation tokens (both 403 on /user) must enable
    # concurrent mode via the surrogate, not collapse to the sequential path.
